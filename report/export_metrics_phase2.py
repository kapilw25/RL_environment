...
def once(args):
    ...
    if args.report:
        ...
        body = ("# Phase 2 results: GRPO + LoRA on Qwen2.5-0.5B\n\n"
                "Auto-generated by report/export_metrics_phase2.py. Updated: %s.\n\n"
                "%s\n\n"
                "Notes: base = pre-RL model, post = after GRPO. The table is empty/flat until the eval "
                "pass finishes (lm-eval writes results only at the end of each pre and post pass). "
                "The delta column is the shipped Phase 2 result (the pre vs post bar race).\n"
                % (ts, md if table else "_(eval not finished yet, no results)_"))
        open(args.report, "w").write(body)
    return len(rows), got
...
def main():
    ...
    ap.add_argument("--report", default=None, help="write/refresh report_phase2.md with the table")
    ...
    if args.loop > 0:
        while True:
            try:
                ...
                import sys
                sys.stderr.write("[export-p2] error: %r\n" % e)
            time.sleep(args.loop)
    else:
        n, got = once(args)
        print(f"[export-p2] {n} rows for {got} -> {args.out}")
